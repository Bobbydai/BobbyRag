...
from setting.models_provider.tools import (
    get_model_by_id,
...
class ApplicationSerializer(serializers.Serializer):
    ...
    class Authentication(serializers.Serializer):
        ...
        authentication_value = serializers.JSONField(
            required=False, allow_null=True, error_messages=ErrMessage.char("认证信息")
        )
    ...
    class Query(serializers.Serializer):
        ...
        select_user_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.uuid("选择用户id")
        )
    ...
    class Operate(serializers.Serializer):
        ...
        def application_list(self, with_valid=True):
            ...
            user_id = self.data.get("user_id")
            application_id = self.data.get("application_id")
            application = Application.objects.filter(user_id=user_id).exclude(
                id=application_id
            )
...
class ContentGenerateSerializer(serializers.Serializer):
    goods_name = serializers.CharField(
        ...
        max_length=64,
        min_length=1,
        error_messages=ErrMessage.char("商品名称"),
    )
    goods_point = serializers.CharField(
        required=False,
        allow_null=True,
        allow_blank=True,
        max_length=256,
    ...
    target_people = serializers.CharField(
        required=False,
        allow_null=True,
        allow_blank=True,
        max_length=256,
    ...
    user_point = serializers.CharField(
        required=False,
        allow_null=True,
        allow_blank=True,
        max_length=256,
    ...
    style = serializers.IntegerField(
        required=False, allow_null=True, error_messages=ErrMessage.char("脚本风格")
    )
    is_ssml_open = serializers.BooleanField(
        required=False, error_messages=ErrMessage.boolean("是否开启ssml")
    )
    ...
    class Operate(serializers.Serializer):
        ...
        def content_generate(self, form_data, with_valid=True):
            ...
            is_ssml_open = form_data.get("is_ssml_open")
            application = (
            ...
            def parse_script_content(content):
                pattern = re.compile(
                    r"<paragraph>\s*<title>(.*?)</title>\s*<content>(.*?)</content>\s*</paragraph>",
                    re.DOTALL,
                )
            ...
            def parse_evalution_content(content):
                pattern = re.compile(
                    r"<paragraph>\s*<title>(.*?)</title>\s*<tag>(.*?)</tag>\s*<content>(.*?)</content>\s*</paragraph>",
                    re.DOTALL,
                )
                matches = pattern.findall(content)
            ...
            final_script, evaluation_result, continuity_phrases = (
            ...
            for eval_item in parsed_evaluation_result:
                matching_script_item = next(
                    (
                        item
                        for item in combined_result["contents"]
                        if item["title"] == eval_item["title"]
                    ),
                    None,
                )
...
class ChooseCommentSerializer(serializers.Serializer):
    ...
    class Operate(serializers.Serializer):
        ...
        def choose_comment(self, form_data, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                comments = form_data.get("comments")
                choose_num = form_data.get("choose_num")
                goods_info = form_data.get("goods_info")
                application = (
                    QuerySet(Application).filter(id=self.data["application_id"]).first()
                )
                model = get_model_by_id(application.model_id, "")
                ...
                generator = LiveStreamScriptGenerator(chat_model)
                comments = generator.generate_comment(comments, choose_num, goods_info)
                pattern = re.compile(
                    r"<content>(.*?)</content>",
                    re.DOTALL,
                )
                matches = pattern.findall(comments)
